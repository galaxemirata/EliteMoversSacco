...
const KaratinaNairobi = () => {

  const navigate = useNavigate()
  const location = useLocation()

  const routeName = location.pathname.replace("/", "").toLowerCase()
    ...
    fetch("http://localhost:5000/api/vehicles")
      ...
      .then(data => {
        ...
        const filtered = data.filter(v =>
          v.route_name?.toLowerCase().replace(/\s/g, "-") === routeName
        )
  ...
  const routeKey =
    vehicle
      ? `${routeName}:${vehicle.number_plate}`
      : routeName
...
export default KaratinaNairobi